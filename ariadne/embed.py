...
def sorted_hmm_paths(hmm_dir: PathLike) -> list[Path]:
    ...
    if not hmm_paths:
        raise FileNotFoundError(
            ...
            "Please check whether the bundled ariadne/hmm directory exists, or pass --tps-hmm-dir explicitly."
        )
...
def _spread_group_coords(coords: np.ndarray, labels: list[str], *, within_scale: float = 0.48) -> np.ndarray:
    ...
    def _gap_multiplier(label_a: str, label_b: str) -> float:
        candidate_labels = {"candidate_ceess", "candidate_non_ceess", "candidate"}
        if label_a in candidate_labels and label_b in candidate_labels:
            return 2.2
        if (label_a in candidate_labels) != (label_b in candidate_labels):
            return 1.35
        return 1.0
    ...
    for _ in range(64):
        ...
        for pair_index, label_a in enumerate(labels_sorted):
            for label_b in labels_sorted[pair_index + 1 :]:
                ...
                min_distance = max(
                    target_gap,
                    radii[label_a] + radii[label_b] + (median_radius * 1.5),
                ) * _gap_multiplier(label_a, label_b)
        ...
        for label in labels_sorted:
            shifted[label] = shifted[label] * 0.985 + centroids[label] * 0.015
        if not moved:
            break
...
def _candidate_group(record: FastaRecord) -> str:
    ...
    if record.metadata.get("is_coral_like") == "yes":
...
def _render_scatter(
    ...
    *,
    ...
    projection_dims: tuple[int, int] = (0, 2),
) -> Path:
    ...
    if len(records) == 0:
        ...
        return target
    required_dims = max(projection_dims) + 1
    if coords.shape[1] < required_dims:
        coords = np.pad(coords, ((0, 0), (0, required_dims - coords.shape[1])))
    ...
    svg.append(f'<text x="{plot_left + (plot_width / 2):.2f}" y="{plot_bottom + 40}" text-anchor="middle" font-size="13" font-weight="600" fill="{axis_color}">{axis_label(dim_x)}</text>')
    y_label_x = outer_left - 54
    y_label_y = plot_top + (plot_height / 2)
    svg.append(
        f'<text transform="rotate(-90,{y_label_x:.2f},{y_label_y:.2f})" x="{y_label_x:.2f}" y="{y_label_y:.2f}" text-anchor="middle" font-size="13" font-weight="600" fill="{axis_color}">{axis_label(dim_y)}</text>'
    )
...
def classify_candidates(
    ...
    *,
    ...
    ceess_classifier: str = "mlp",
    ceess_epochs: int = 200,
    ceess_hidden_dim: int = 128,
    ceess_representation_dim: Optional[int] = None,
    ceess_projection_dim: Optional[int] = None,
    ceess_dropout: float = 0.1,
    ceess_learning_rate: float = 1e-3,
    ceess_weight_decay: float = 1e-4,
    ceess_train_batch_size: int = 8,
    ...
    if ceess_xlsx is not None:
        ...
        if coral_like_records and ceess_xlsx_path.exists():
            try:
                from ariadne.model import classify_ceess_candidates_with_esm
                ...
                ceess_result = classify_ceess_candidates_with_esm(
                    ...
                    classifier_kind=ceess_classifier,
                    epochs=ceess_epochs,
                    hidden_dim=ceess_hidden_dim,
                    representation_dim=ceess_representation_dim,
                    projection_dim=ceess_projection_dim,
                    dropout=ceess_dropout,
                    learning_rate=ceess_learning_rate,
                    weight_decay=ceess_weight_decay,
                    train_batch_size=ceess_train_batch_size,
                    barlow_redundancy_weight=ceess_barlow_redundancy_weight,
                    classifier_checkpoint=ceess_mlp_checkpoint,
                )
    ...
    candidate_row_by_id = {str(row["sequence_id"]): row for row in candidate_rows}
    cluster_context_rows = []
    for candidate_index in range(reference_count, len(all_records)):
        candidate = all_records[candidate_index]
        crow = candidate_row_by_id[candidate.id]
        cluster_context_rows.append(
            {
                "sequence_id": candidate.id,
                "candidate_group": _candidate_group(candidate),
                "predicted_source": str(crow["predicted_source"]),
                "nearest_reference_source": str(crow["nearest_reference_source"]),
                "nearest_distance": round(float(crow["nearest_distance"]), 6),
